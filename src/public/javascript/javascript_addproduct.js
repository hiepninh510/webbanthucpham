...
//     if (isNaN(priceOld) || isNaN(priceNew) || priceOld < 0 || priceNew < 0 || !(/^\d+(\.\d{3})?$/.test(priceOld)) || !(/^\d+(\.\d{3})?$/.test(priceNew))) {
...
document.querySelector('form').addEventListener('submit', function(event) {
    ...
    // Kiểm tra giá gốc và giá mới
    if (isNaN(priceOld) || isNaN(priceNew) || priceOld < 0 || priceNew < 0 || !(/^\d+(\.\d{1,3})?$/.test(priceOld)) || !(/^\d+(\.\d{1,3})?$/.test(priceNew)) || !/^\d+(\.\d{1,3})?$/.test(priceOld.replace('.', ''))) {
        alert('Giá sản phẩm phải là số không âm và có định dạng hợp lệ (ví dụ: 1000.000).');
        event.preventDefault();
        return;
    }
